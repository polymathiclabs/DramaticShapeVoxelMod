...
-- full battle screen.
OverworldBattle.HUD_SCALE = 0.32
...
OverworldBattle.HUD_SCALE_POV = 0.21
...
OverworldBattle.HUD_LIFT = 44
OverworldBattle.HUD_LIFT_ENEMY = 60
OverworldBattle.HUD_LIFT_POV = 14
OverworldBattle.HUD_LIFT_ENEMY_POV = 22
...
local function projectedHudRect(side, source, shot, scale)
  ...
  if shot.pov then
    lift = side == "enemy" and OverworldBattle.HUD_LIFT_ENEMY_POV
                           or OverworldBattle.HUD_LIFT_POV
  else
    lift = side == "enemy" and OverworldBattle.HUD_LIFT_ENEMY
...
function OverworldBattle.install()
  ...
  -- Move animations are authored against the pics' fixed slots, but the mons
  -- are now standing at their projected world positions. Map the two classic
  -- mon anchors onto those two projected points so a strike, ball toss, or
  -- impact stays over the Pokemon it belongs to instead of merely following
  -- the pair's midpoint.
